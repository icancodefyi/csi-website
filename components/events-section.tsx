...
const events = [
  ...
  },
  {
    ...
    desc: "Hear from tech leaders, startup founders, and engineers about what it takes to build products that matter.",
    color: "#0a1a30",
    accent: "#3b82f6",
  },
  {
  ...
  },
  {
    ...
    desc: "Collaborative deep-work sessions. Code along, ask questions, and leave with a working project — not a tutorial.",
    color: "#061428",
    accent: "#818cf8",
  },
];
...
function getOverflow(cardCount: number): number {
  ...
  const cardWidth = w * (isMobile ? 0.78 : w < 1024 ? 0.4 : 0.28);
  const gap = isMobile ? 16 : 24;
  const padLeft = isMobile ? 16 : 32;
  const padRight = isMobile ? 32 : 48;
...
export default function EventsSection() {
  ...
  return (
    <section
      id="events"
      ref={targetRef}
      className="relative h-[350vh] bg-csi-cream"
    >
      <div className="sticky top-0 h-screen flex flex-col justify-center overflow-hidden pt-14 md:pt-18 pb-6 md:pb-8">
        ...
        <div className="mb-8 md:mb-12 px-4 md:px-6 w-full max-w-5xl mx-auto">
          <span className="text-xs font-medium tracking-[0.2em] uppercase text-csi-navy/25 mb-6 block">
            What we do
          </span>
            ...
            {events.map((event, i) => (
              ...
              >
                ...
                <div className="absolute top-6 md:top-8 left-6 md:left-8 z-10 select-none">
                  <span className="text-[6rem] md:text-[8rem] font-black leading-none -tracking-[0.06em] opacity-[0.03] text-white">
